import com.dkanen.rotationrumpus.game.*
import com.dkanen.rotationrumpus.game.actor.*
import com.dkanen.rotationrumpus.game.ecs.components.graphics.renderModel
import com.dkanen.rotationrumpus.game.graph.JsonGraphNodeList
import com.dkanen.rotationrumpus.game.graph.Positioner
import com.dkanen.rotationrumpus.math.*
import com.dkanen.rotationrumpus.openrndr.ArgsHolder
...
fun main(args: Array<String>) = application {
    ...
    program {
        ...
        // The beginning of the FPS interval measured in seconds.
        var fpsIntervalStartSeconds = seconds
        // The number of frames in the interval.
        var fpsIntervalFrameCount = 0
        // The length of the FPS interval in seconds.
        val fpsSampleSeconds = 1.0
        // The last calculated FPS.
        var fpsCurrentFps = "0"

        val graph: JsonGraphNodeList = JsonGraphNodeList(listOf())
        ...
        extend {
            ...
            val seconds = application.seconds
            val secondsSinceLastFrame = seconds - lastFrameTime
            val timeStep = min(maximumTimeStep, secondsSinceLastFrame)
            ...
            // draw frame rate
            if (seconds - fpsIntervalStartSeconds > fpsSampleSeconds) {
                fpsIntervalStartSeconds = seconds
                fpsCurrentFps = (fpsIntervalFrameCount / fpsSampleSeconds).toInt().toString()
                fpsIntervalFrameCount = 0
            }
            drawer.defaults()
            drawer.fill = ColorRGBa.GREEN
            drawer.fontMap = fontBook.medium
            drawer.text("$fpsCurrentFps fps", 11.0, 11.0)
